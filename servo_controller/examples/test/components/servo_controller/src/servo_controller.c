...
static const rmt_symbol_word_t test_signal[8] = {{.val = 0x80020002},{.val = 0x80020002},{.val = 0x80020002},{.val = 0x80020002},
                                        {.val = 0x80020002},{.val = 0x80020002},{.val = 0x80020002},{.val = 0x8002000A}};
...
//Send out a test signal alternating high and low
esp_err_t test_board(servo_controller_handle_t handle){
    ESP_RETURN_ON_FALSE(handle != NULL, ESP_ERR_INVALID_ARG, TAG, "The handle is null");
    ESP_RETURN_ON_FALSE(handle->rmt_handles[0] == NULL, ESP_ERR_INVALID_ARG, TAG, "The handle hasn't been initialized");
    ESP_RETURN_ON_FALSE(!handle->running, ESP_ERR_INVALID_STATE, TAG, "The servo_controller is started");

    rmt_transmit_config_t transmit_config = {.loop_count = 1, .flags.eot_level = 0, .flags.queue_nonblocking = false};

    rmt_transmit(handle->rmt_handles[0], handle->encoder_handle, clk, 17*sizeof(rmt_symbol_word_t), &transmit_config);
    rmt_transmit(handle->rmt_handles[1], handle->encoder_handle, &pulse, 1*sizeof(rmt_symbol_word_t), &transmit_config);
    rmt_transmit(handle->rmt_handles[2], handle->encoder_handle, test_signal, 8*sizeof(rmt_symbol_word_t), &transmit_config);
    rmt_transmit(handle->rmt_handles[3], handle->encoder_handle, test_signal, 8*sizeof(rmt_symbol_word_t), &transmit_config);
